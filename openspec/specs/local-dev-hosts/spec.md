...
How sku treats local development hostnames that are exact `localhost` or end with `.localhost`.
Those names are exempt from missing-hosts-file warnings.
`sku setup-hosts` still writes `.localhost` entries.
Local HTTPS remains available.
In-repo examples prefer `*.localhost` hostnames.
