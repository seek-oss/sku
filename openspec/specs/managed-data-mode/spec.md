...
Sku MUST create each site’s React Router static handler once at init.
Sku MUST NOT call `createStaticHandler` on the per-request path.
Per request sku only selects the pre-built handler and calls `query()` / `createStaticRouter`.

Sku MUST NOT wrap or otherwise modify the route tree to mount consumer providers.
Provider mounting happens outside the router.
See the request-exports requirement.
...
Streaming data transports must inject serialized state into the response between React's stream chunks so it executes before hydration.
Sku owns the render call and the response pipe.
Sku MUST provide that seam.
...
Sku MUST NOT add Jest transforms for `react-router` / `cookie-es` / `import.meta` in this change.
SSR requires Vitest.
React Router 8 + Jest is out of scope.
