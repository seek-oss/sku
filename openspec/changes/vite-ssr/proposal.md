...
Sku owns `renderToPipeableStream` and the response pipe, so an app cannot reach the stream; this is the same reason sku owns the request-scoped value seam (`SkuSsrProvider` + entry getters) rather than letting apps wrap the Document render.
...
Sku always mounts `SkuSsrProvider` and exposes typed hooks via `createSkuSsrContexts<typeof server, typeof client>()`; env-differing values (e.g. `apiClient` / Apollo `makeClient`) come from dual-entry `getReactContext`; isomorphic provider mounts (Apollo, Vocab) live in the app’s root layout route.
...
- Request-entry contract: `serverEntry` / `clientEntry` each **`export default`** an object from `defineServerEntry` / `defineClientEntry` (zero-runtime inference helpers; structural types `SkuSsrServerEntry` / `SkuSsrClientEntry`).
  ...
  Sku owns Document, streaming, assets, CSP headers, and an always-on `SkuSsrProvider` outside the router (`Document` → `SkuSsrProvider` → router).
...
- No app-authored dual-entry `Providers` / `SkuSsrProvidersProps`.
...
- `useInsertHtml()` on the existing browser-safe `sku/ssr` subpath (alongside `usePreloadRoute` and `createSkuSsrContexts`) so app-owned streaming data transports can insert HTML into the response stream.
...
- **Public API:** `buildType`; `routesEntry` (default `src/routes.tsx`); single `port` (no `serverPort`); optional config `sites` for Vite SSR (empty/omitted soft-defaults to `'default'`); optional `expressTrustProxy` (boolean → Express hop count `1`); `routesEntry` named export `routes`; `SkuSsrRouteObject` type helper (`sites?`); default-exported request-entry objects via `defineServerEntry` / `defineClientEntry` (`SkuSsrServerEntry` / `SkuSsrClientEntry`); optional getters `getSite` / `getLanguage` / `getClientContext` / `getReactContext` (sibling projection; `defineServerEntry` infers `Site` / `Language` / `ClientContext` / `ReactContext`; `defineClientEntry<typeof server>` extracts `Site` / `ClientContext` from the server entry; `getSite` required when config has >1 site); optional `middleware` / `onListen` / `onHydrate` / dual-entry `getRouterContext`; ALS CSP nonce; server `language` for Document vocab; always-on `SkuSsrProvider` + `createSkuSsrContexts<typeof server, typeof client>()` (`useSite` typed from `getSite`, `useClientContext` / `useReactContext`); create template `vite-ssr` (MAY omit `sites`; `expressTrustProxy: true`); reused `devServerMiddleware`; `usePreloadRoute` and `useInsertHtml` on `sku/ssr`.
- **Deps:** `react-router` optional peer `^8`; Express 4 shared with webpack SSR (no 4 → 5 bump).
- **Docs / release:** Product + Migrating docs (incl. `routesEntry`, multi-site flat `routes` + `sites` + `getSite`, `define*Entry` + `createSkuSsrContexts<typeof …>`, three value channels + root layout wrapping, `onListen`, `expressTrustProxy`, webpack `onStart` → `onListen`); related config/CSP/Vite docs; data-loading hierarchy + red warning against `req` in context; Data Mode dual `getRouterContext` patterns (including client-nav ≠ initial SSR location); Apollo streaming hydration walkthrough (`useInsertHtml`, `getReactContext` + root-layout provider, nonce on injected scripts, cache reuse across hydration); experimental warning + changeset; Express 5 deferred.
...
- **Module identity (published installs):** App hooks via `sku/ssr` and sku’s `SkuSsrProvider` / insert-html / preload / nonce storage MUST share one module instance. Design Decision 26: consolidate sku runtime imports onto `sku/ssr` **and** `optimizeDeps.exclude` for `'sku'` / `'sku/ssr'` (tsdown `unbundle: true` alone is insufficient under Vite dep-optimization). Covered by a Node assert + existing vite-ssr browser tests; dedicated tarball e2e deferred.
