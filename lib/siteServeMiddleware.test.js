...
describe('siteServeMiddleware', () => {
  ...
  describe('when sites are configured', () => {
    ...
    test('should match site to host and route', () => {
      middleware(
      ...
      expect(mockResponse.send).toBeCalledWith('seekAnz/home');
    });
    ...
    test('should match site to host and route', () => {
      ...
      expect(mockNext).toBeCalledTimes(0);
      expect(mockResponse.send).toBeCalledWith('jobStreet/home');
    });

    test('should fallback to first site if no matching host', () => {
      middleware({ hostname: 'localhost', path: '/' }, mockResponse, mockNext);
    ...
    test('should match dynamic routes', () => {
      ...
      expect(mockNext).toBeCalledTimes(0);
      expect(mockResponse.send).toBeCalledWith('jobStreet/details/:id');
    });

    test('should match dynamic routes with host fallback', () => {
      middleware(
      ...
      expect(mockNext).toBeCalledTimes(0);
      expect(mockResponse.send).toBeCalledWith('seekAnz/serp/:id/info');
    });
  });
  ...
  describe('when no sites are configured', () => {
    ...
    test('should match route', () => {
      ...
      expect(mockNext).toBeCalledTimes(0);
      expect(mockResponse.send).toBeCalledWith('home');
    });
    ...
    test('should match dynamic routes', () => {
      ...
      expect(mockNext).toBeCalledTimes(0);
      expect(mockResponse.send).toBeCalledWith('details/:id');
    });
  });
});
