...
Previously, `sku lint <path>` ran ESLint and Prettier on the given paths but still ran TypeScript across the *entire project*. This is because TypeScript ignores your `tsconfig.json` when filenames are passed explicitly, so the check can't be scoped to a single path. This unexpected behaviour has now been removed.
