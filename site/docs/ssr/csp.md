...
On SSR, Content Security Policy is delivered as **HTTP headers** (`Content-Security-Policy` / `Content-Security-Policy-Report-Only`).
Meta `http-equiv` CSP is not used on this path.
