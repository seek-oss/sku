...
- **Fixtures/tests:** Vite SSR fixture (streaming, CSP, entries, per-route chunks, vocab); create template tests. Config edge cases (webpack + `ssr`, `-ssr` with `buildType`, absolute/`CDN` `publicPath`) stay config/command validation — no dedicated browser e2e fixtures
- **Opt-in / adopt:** via `buildType`; document hydrate (not `#app`); relative `publicPath` only; required named exports; shared `__sku_alias__*` entry aliases with static Vite
