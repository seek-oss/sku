...
describe('selectForSite', () => {
  it('returns the site tree when present', () => {
    expect(selectForSite(siteRouteTrees, 'au', 'onRequest')).toEqual([
      { path: '/au-only' },
    ]);
  });

  it('hard-errors when site is missing or not a string', () => {
    expect(() => selectForSite(siteRouteTrees, undefined, 'onRequest')).toThrow(
      /must provide a non-empty string 'site'/,
    );
    expect(() => selectForSite(siteRouteTrees, '', 'onRequest')).toThrow(
      /must provide a non-empty string 'site'/,
    );
  ...
  it('hard-errors when there is no pre-built tree for site', () => {
    expect(() => selectForSite(siteRouteTrees, 'uk', 'onRequest')).toThrow(
  ...
  it('hard-errors for inherited Object properties rather than returning them', () => {
    expect(() =>
      selectForSite(siteRouteTrees, 'constructor', 'onRequest'),
