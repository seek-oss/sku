...
import { accent, error, strong } from '@sku-private/utils/console';
...
export const serveAction = async ({
  ...
  if (preferredSite && !availableSites.some((site) => preferredSite === site)) {
    console.error(error(`Unknown site '${strong(preferredSite)}'`));
  ...
  if (
    ...
    console.error(
      error('sku serve not supported when publicPath is on a separate domain.'),
    );
    process.exit(1);
  }
