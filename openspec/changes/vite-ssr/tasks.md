...
## 21. Async `getClientContext` / `getReactContext`

- [x] 21.1 Type `getClientContext` and dual-entry `getReactContext` as `T | Promise<T>`.
- [x] 21.2 Unwrap with `Awaited` in extractors, `define*Entry` sibling args, and `createSkuContexts` hooks.
- [x] 21.3 Keep `getSite` / `getLanguage` and client `getRouterContext` synchronous.
...
- [x] 21.6 Type-level tests: Promise returns unwrap for hooks and sibling args. Remaining sync getters still type-check.
