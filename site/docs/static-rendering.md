...
> [!NOTE]
> `sku start` will default to the first `environment` and `site` in your config if provided. You can use any site by passing the `--environment` argument e.g. `sku start --environment production`
...
> [!IMPORTANT]
> Make sure to wrap your app with the `SkuProvider`. This is **required** for the app to work.
...
> [!NOTE]
> **Experimental:** `renderApp` provides a `renderToStringAsync` function parameter which can be used instead of calling `React.renderToString`.
