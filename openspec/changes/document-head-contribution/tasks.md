...
- [x] 1.3 Do not export `HeadAssets` / `DocumentAssetLinks` from `sku/runtime`
- [x] 1.4 Mount `DocumentAssetLinks` beside the router on stream and hydrate

## 2. App-owned html

- [x] 2.1 Stop wrapping `createDocumentAttempt` and `ssr-client` in sku `Document`
- [x] 2.2 Template and fixtures render `<html>` / `<head>` / `<body>` without `HeadAssets`
- [x] 2.3 `ErrorBoundary` on a child of the html route

## 3. Tests and docs

- [x] 3.1 Cover sku links in app `<head>` with `precedence`, no wrapping sku `<html>`, no app `HeadAssets`
- [x] 3.2 Cover non-hoistable `<style>` in `<head>` under a provider that wraps `<html>`
- [x] 3.3 Cover child-route `ErrorBoundary` keeps `<html>`
- [x] 3.4 Docs: root-layout document, sku-hoisted assets, inner `ErrorBoundary`, drop HeadAssets
