...
This release changes the way sku handles rendering html & asset generation for both server rendered and statically rendered apps.

Please read through [static-rendering](../static-rendering) before attempting migration.

## Static apps

Build output folder structure has changed in some circumstances. After making the following changes, please check your `target` directory and update any testing and deployment strategies to suit. See [static-rendering](../static-rendering) for details on the new structure.

### render

If your app renders multiple pages/routes then first open up your `sku.config.js`.
Using the new `routes` option, detail all your applications routes.
