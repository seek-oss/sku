...
- Sku syncs its recommended pnpm settings into the project's existing `pnpm-workspace.yaml` whenever `configureApp` runs (configuration-enabled sku commands and postinstall), using YAML edits that preserve unrelated content:
  ...
  - Ownership is tracked with a `[sku_managed]` comment marker on sku-written values and sku-owned collection entries, written after any explanatory comment (`# 3 days [sku_managed]`) and detected anywhere in a comment.
    ...
    Comments on adopted or overwritten sku-managed entries are replaced with the sku marker (and any sku explanatory comment).
...
- `sku configure` runs the full enforcing sync: managed single-value settings are overwritten with sku's current defaults in both directions (managed means enforced; no never-downgrade or strength-ordering special cases), marked keys in object settings are aligned with sku's defaults, and retired sku-owned entries are removed.
  Manual invocation is intentional, so enforcement only happens on explicit request.
  Removal is scoped strictly to entries still carrying a `[sku_managed]` marker: if the user deletes an entry's marker it becomes user-owned, and because a retired entry no longer matches a default it is never re-adopted, so it is never removed.
...
- Consumer projects: on first run of an upgraded sku, `pnpm-workspace.yaml` gains any missing static settings and markers and loses the `configDependencies` entry.
  No existing values are changed by the automatic sync.
...
- Escape hatches:
  ...
  - A retired entry is only removed while it carries its `[sku_managed]` marker; delete the marker (or re-add the entry afterwards) to keep it.
- Tests:
