...
To opt out of workspace management entirely, set `managedWorkspace: false` in your sku config. This skips the `sku lint` check and the `sku format` sync (including the `pnpm-plugin-sku` migration), for projects that keep their `pnpm-workspace.yaml` but manage their own pnpm settings. It defaults to `true` and does not affect the explicit `sku configure workspace` subcommand.
